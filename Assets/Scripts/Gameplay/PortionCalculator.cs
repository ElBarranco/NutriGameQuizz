...
public static class PortionCalculator
{
    ...
    {
        if (sel.Grams > 0f) return sel.Grams;
    ...
    public static float ComputeValueForPortion(FoodData f, float grams, QuestionSubType sub)
    {
        return GetPer100(f, sub) * (Mathf.Max(0f, grams) / 100f);
    }

    // Utile quand tu as déjà un PortionSelection
    public static float ComputeValue(FoodData f, PortionSelection sel, QuestionSubType sub, float defaultPieceWeightG = 120f, float densityGPerMl = 1f)
    {
        float g = ToGrams(sel, defaultPieceWeightG, densityGPerMl);
        return ComputeValueForPortion(f, g, sub);
    }
